...
public class LoginController {
	...
	{
		System.out.println(pass.getMob()+" "+pass.getPassword());
		String status="";
		
		
			status=doc.forgetpassdoctor(pass.getMob(),pass.getPassword());
			if(status!="")
			{
				return status;	
			}else
	...
	public String forgetpasswordpatient(@RequestBody Forgetpassword pass)
	{
		...
		for(int i=0;i<list.size();i++)
		{
			if(list.get(i).getMob_number()==pass.getMob())
			{
				...
				status=patient.forgetpasspatient(id,pass.getPassword());
				break;
